...
def run_environment_check():
    print("=" * 60)
    print("MDRIIA GROUP 04 - TOOLCHAIN AND PHYSICS VERIFICATION")
    print("=" * 60)
    ...
    try:
        ...
        xml_path = os.path.join(os.path.dirname(current_dir), "models", "space_debris_gripper.xml")
        ...
        if os.path.exists(xml_path):
            ...
            print(f"[INFO] Microgravity Flag: gravity={model.opt.gravity}")
        else:
            print(f"[WARNING] Local XML not found at {xml_path}. Testing fallback string.")
            fallback = "<mujoco><option gravity='0 0 0'/><worldbody><body name='b'><geom type='sphere' size='0.1'/></body></worldbody></mujoco>"
            m = mujoco.MjModel.from_xml_string(fallback)
            d = mujoco.MjData(m)
            mujoco.mj_step(m, d)
            print("[SUCCESS] Fallback microgravity physics step verified.")
    except ImportError:
